...
url = 'https://management.azure.com/subscriptions/' + subscription_id + '/providers/Microsoft.Web/certificates'
# Run GET
...
for i in range(len(parse_data['value'])):
    # Get date expired
    date_expired = parse_data['value'][i]['properties']['expirationDate']
...
for i in list_expired:
    out = "- Domain: " + parse_data['value'][i]['properties']['subjectName'] + " | " + parse_data['value'][i]['properties']['expirationDate']
    message += out + "\n"
    print(out)
...
for i in list_warning:
    diff_day = days_between_now(parse_data['value'][i]['properties']['expirationDate'])
    out = "- Domain: " + parse_data['value'][i]['properties']['subjectName'] + " | " + parse_data['value'][i]['properties']['expirationDate'] + " | " + str(diff_day) + " days remaining"
    message += out + "\n"
    print(out)
